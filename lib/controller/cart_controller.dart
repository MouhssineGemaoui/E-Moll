...
import 'package:EMOOL/data/model/couponmodel.dart';
...
class CartController extends GetxController {
  ...
  goToPageCheckout() {
    if (data.isEmpty) return Get.snackbar("100".tr, "66".tr);
  ...
  delete( itemsid) async {
    ...
    if (StatusRequest.success == statusRequest) {
      ...
      if (response['status'] == "success") {
        Get.snackbar(
           "102".tr,"67".tr,icon: Icon(Icons.notifications_active_outlined),
             duration: Duration(seconds: 1));
        // data.addAll(response['data']);
      } else {
        statusRequest = StatusRequest.failure;
  ...
  checkcoupon() async {
    ...
    if (StatusRequest.success == statusRequest) {
      ...
      if (response['status'] == "success") {
        ...
        couponname = null;
        couponid = null;
        Get.snackbar("102".tr, "Coupon Not Valid",
        icon: Icon(Icons.notifications_active_outlined),
